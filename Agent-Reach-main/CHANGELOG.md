...
### 🕷️ MediaCrawler 集成 / MediaCrawler integration

- **移植 MediaCrawler 核心到 `mediacrawler/` 子树：** 抖音、快手、微博、百度贴吧、知乎五个登录态平台渠道（小红书/B站 MediaCrawler 也支持，主项目已有对应渠道）。Playwright 保留登录态 + JS 表达式取签名，无需逆向；登录态缓存在 `mediacrawler/browser_data/`。该子树适用 NON-COMMERCIAL LEARNING LICENSE（禁止商用）
- **新增 `agent-reach crawl <platform>` 命令：** 参数原样透传 `mediacrawler/main.py`，自动使用 `mediacrawler/.venv` 解释器；依赖安装 `cd mediacrawler && uv venv .venv && uv pip install -r requirements.txt`
- **doctor 新增 5 个渠道体检：** 集成存在性 → 依赖就绪 → 登录态缓存三段检查，不启动浏览器（登录是交互式扫码）

### 🖥️ Cross-platform / 跨平台

- **Windows/Linux 支持：** sidecar 二进制名按平台选择（`tlsproxy` / `tlsproxy.exe`）；`detect/self_check.py` 的 bundled Chromium 查找按平台定位 ms-playwright 缓存（Windows `AppData/Local`、Linux `~/.cache`、macOS `Library/Caches`），支持 `PLAYWRIGHT_BROWSERS_PATH` 覆盖
- **CI：** 测试矩阵扩展为 ubuntu-latest + windows-latest × Python 3.10–3.13
- **修复 9 个 Windows 遗留测试失败：** symlink 测试无权限时 skip；`make_private_dir` 按 `os.name` 分支；卸载输出路径 normpath；测试补 `USERPROFILE` 隔离

### 🐛 Bug Fixes / 修复

- **Node.js 安装块增加 OS 判断：** 原先无条件执行 NodeSource apt 流程，macOS/Windows 静默失败；现按平台分支（Linux → NodeSource apt，macOS → brew，Windows → winget）
- **SSRF 防护补 DNS 解析校验：** 原先只检查字面 host，公网域名解析到内网 IP 可绕过；现对非 IP 字面量做 `getaddrinfo` 解析并校验所有结果

---

## [1.5.0] - 补记（reconstructed）

> ⚠️ 本节为补记：1.4.0–1.5.0 发布时未维护本文件，以下依据 v1.5.0 代码与文档重建；1.4.0 的具体变更记录已缺失。
> This entry was reconstructed from the v1.5.0 codebase and docs; the 1.4.0 record is lost.

### 🥷 Stealth 浏览器渲染能力 / Stealth browser rendering

- **新增顶层 `stealth_kit` 包：** 指纹档案（`FingerprintProfile` + 跨面一致性校验）、6 个 init-script 注入（webdriver/plugins/permissions/webgl/canvas/navigator）、CDP 协议层覆盖、行为模拟（贝塞尔鼠标/对数正态打字）、会话池；只用 Playwright 公开 API
- **Go uTLS sidecar（`sidecar/`）：** 本地 MITM 代理以 Chrome ClientHello 重发 TLS，改写 JA3/JA4；支持 http/socks5 egress proxy chaining
- **web 渠道兜底：** Jina Reader 失败（需 JS 渲染/反爬）自动回退 stealth 浏览器；`web_backend=stealth` 可强制；playwright 为可选依赖 `[browser]`
- **OpenCLI 成为桌面首选后端**（见 docs/update.md）
- **检测工具：** `detect/self_check.py` 19 项自检（12 指纹 + 7 行为）、`detect/collect_profile.py` 真实指纹采集
