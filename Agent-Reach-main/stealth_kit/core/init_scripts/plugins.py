...
JS logic extracted verbatim from stealth-kit/src/core/init-scripts/plugins.ts
(TypeScript type annotations stripped, logic unchanged).
"""

PLUGINS_INIT_SCRIPT = r"""
// Makes a function look like a native method when toString()'d.
const nativeCode = (name) =>
...
const plugins = pluginData.map(p => {
  ...
  Object.defineProperties(plugin, {
    ...
    item: { value: disguise((i) => mimeTypes[i], 'item') },
    namedItem: { value: disguise((n) => mimeTypes.find(m => m.type === n), 'namedItem') },
  });
...
Object.defineProperties(pluginArray, {
  ...
  item: { value: disguise((i) => plugins[i], 'item') },
  namedItem: { value: disguise((n) => plugins.find(p => p.name === n), 'namedItem') },
...
def make_plugins_init_script() -> str:
    return PLUGINS_INIT_SCRIPT
