...
| Whole-site / batch scraping (firecrawl) | web | [references/web.md](references/web.md) |
...
## Site-level scraping (firecrawl, requires setup)

Available when doctor's web channel shows the firecrawl stack as reachable; skip this section otherwise:

- List all URLs of a site: `agent-reach map <site-url> [--limit N]`
- Crawl an entire site to markdown: `agent-reach crawl-site <site-url> [--limit N]` (results written as JSONL under the acquisition data dir)
- Batch-scrape a URL list: `agent-reach batch <file-with-one-url-per-line>`
- Search: `agent-reach search <query> [--limit N]` (requires SearXNG on the stack)

Single-page reading priority is unchanged: Jina Reader → firecrawl → stealth. If firecrawl hits an anti-bot challenge, it falls back to stealth automatically — no manual intervention needed.
For recurring whole-site sources: use `kind: firecrawl_crawl` with an entry url in sources.yaml (see config/sources.example.yaml).
...
# Douyin / Kuaishou / Weibo / Tieba / Zhihu (vendored MediaCrawler; login
# state cached under mediacrawler/browser_data/, first run opens QR login)
agent-reach crawl dy --type search --keywords "query"          # Douyin search
agent-reach crawl dy --type creator --creator_id <sec_uid>     # Douyin creator feed
agent-reach crawl wb --type search --keywords "query"          # Weibo
agent-reach crawl zhihu --type search --keywords "query"       # Zhihu
# other platform codes: ks=Kuaishou tieba=Tieba (xhs/bili also supported,
# but agent-reach already has dedicated channels for them)
agent-reach crawl dy --help    # full passthrough options (comments, limits, --lt qrcode/phone/cookie)
```
