...
| 整站/批量抓取（firecrawl） | web | [references/web.md](references/web.md) |
...
# 抖音 / 快手 / 微博 / 贴吧 / 知乎（内置 MediaCrawler，登录态缓存在
# mediacrawler/browser_data/；首次运行打开浏览器扫码登录）
agent-reach crawl dy --type search --keywords "query"          # 抖音关键词搜索
agent-reach crawl dy --type creator --creator_id <sec_uid>     # 抖音创作者主页
agent-reach crawl wb --type search --keywords "query"          # 微博
agent-reach crawl zhihu --type search --keywords "query"       # 知乎
# 其他平台代码：ks=快手 tieba=贴吧（xhs/bili 也支持，但主项目已有对应渠道）
# 更多参数（评论数、条数、登录方式 --lt qrcode/phone/cookie 等）：
agent-reach crawl dy --help
```
