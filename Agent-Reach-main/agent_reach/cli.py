...
def main():
    ...
    p_crawl = sub.add_parser(
        "crawl",
        help="Run a MediaCrawler platform crawl (logged-in Chinese platforms)",
        description="Forward to the vendored MediaCrawler subtree. Extra arguments "
                    "are passed through to mediacrawler/main.py, e.g.:\n"
                    "  agent-reach crawl dy --lt qrcode --type creator --creator_id <sec_uid>\n"
                    "  agent-reach crawl wb --type search --keywords 财经\n"
                    "First crawl opens a browser for QR login; the login state is "
                    "cached under mediacrawler/browser_data/.",
    )
    p_crawl.add_argument("platform",
                         choices=["xhs", "dy", "ks", "bili", "wb", "tieba", "zhihu"],
                         help="MediaCrawler platform code")
    p_crawl.add_argument("mc_args", nargs=argparse.REMAINDER,
                         help="Arguments passed through to MediaCrawler main.py")
            ...
            if shutil.which("brew"):
                try:
                    ...
                    if shutil.which("node"):
                        ...
                        print("  [!]  Node.js install failed. Try: brew install node, or download from https://nodejs.org")
                except Exception:
                    print("  [!]  Node.js install failed. Try: brew install node, or download from https://nodejs.org")
            else:
                print("  [!]  Node.js not found. Install: brew install node (requires Homebrew), or download from https://nodejs.org")
        elif os_type == "windows":
            if shutil.which("winget"):
                try:
                    subprocess.run(["winget", "install", "OpenJS.NodeJS.LTS"], capture_output=True, timeout=300)
                    if shutil.which("node"):
                        print("  ✅ Node.js installed")
                    else:
                        print("  [!]  Node.js install failed. Try: winget install OpenJS.NodeJS.LTS, or download from https://nodejs.org")
                except Exception:
                    print("  [!]  Node.js install failed. Try: winget install OpenJS.NodeJS.LTS, or download from https://nodejs.org")
            else:
...
def _cmd_crawl(args):
    """Forward a crawl request to the vendored MediaCrawler subtree."""
    from agent_reach import mediacrawler_bridge as bridge

    if not bridge.mediacrawler_available():
        print("[X] MediaCrawler 未集成（mediacrawler/ 目录缺失）")
        sys.exit(1)
    if not bridge.deps_ready():
        print(f"[X] {bridge.INSTALL_HINT}")
        sys.exit(1)
    sys.exit(bridge.run_crawl(args.platform, args.mc_args))
...
def _cmd_uninstall(args):
    ...
    legacy_credential_paths = (
        # normpath so Windows prints a clean backslash path instead of the
        # mixed-separator expanduser result; a no-op on POSIX.
        os.path.normpath(os.path.expanduser("~/.config/xfetch/session.json")),
        os.path.normpath(os.path.expanduser("~/.config/bird/credentials.env")),
    )
