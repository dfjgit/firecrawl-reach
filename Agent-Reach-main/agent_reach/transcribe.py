...
def _assert_safe_public_url(url: str) -> None:
    """Reject URLs whose host is — or DNS-resolves to — a local/internal address."""
    if "://" not in url:
    ...
    try:
        ...
        return  # IP literal already checked above
    except ValueError:
        pass
    # Hostname: a public-looking name may still resolve to an internal IP.
    # Fail closed — an unresolvable host cannot be verified as public.
    try:
        resolved = socket.getaddrinfo(host, None)
    except socket.gaierror:
        raise TranscribeError("SSRF blocked: host could not be resolved")
    if any(_is_private_ip(info[4][0]) for info in resolved):
        raise TranscribeError(
            "SSRF blocked: host resolves to a private/internal address"
        )
