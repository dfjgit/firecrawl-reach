...
def make_private_dir(path: str | Path) -> Path:
    ...
    # Gate on os.name (real OS capability) rather than sys.platform: on
    # Windows os.open() cannot open a directory, so the fd-based recheck
    # would always fail even though sys.platform may be patched in tests.
    if os.name != "nt":
        flags = (
            os.O_RDONLY
