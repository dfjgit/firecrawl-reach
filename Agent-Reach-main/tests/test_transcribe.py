...
class TestDownloadAudioSafety:
    ...
    def _public_dns(self, monkeypatch):
        """Keep DNS hermetic: public hostnames resolve to a public IP."""
        import socket
    ...
    def test_rejects_hostname_resolving_to_private_ip(self, monkeypatch, tmp_path):
        import socket

        monkeypatch.setattr(tr, "_require", lambda binary: None)

        def should_not_run(*args, **kwargs):
            raise AssertionError("yt-dlp must not run for internal hosts")
        ...
        with pytest.raises(tr.TranscribeError, match="private|internal|SSRF"):
            tr.download_audio("https://evil.example.com/latest/meta-data/", tmp_path)

    def test_rejects_hostname_when_dns_resolution_fails(self, monkeypatch, tmp_path):
        import socket

        monkeypatch.setattr(tr, "_require", lambda binary: None)

        def should_not_run(*args, **kwargs):
            raise AssertionError("yt-dlp must not run for unverifiable hosts")
        ...
        with pytest.raises(tr.TranscribeError, match="SSRF"):
            tr.download_audio("https://nonexistent.invalid/audio", tmp_path)

    def test_passes_hostname_resolving_to_public_ip(self, monkeypatch, tmp_path):
        monkeypatch.setattr(tr, "_require", lambda binary: None)
        captured = {}
