...
def _run_node_install(monkeypatch, system, extra_tools=()):
    """Run _install_system_deps with gh present and node/npm missing."""
    import platform
    import shutil
    ...
    monkeypatch.setattr(
        ...
        if name == "gh"
        else (f"/usr/bin/{name}" if name in extra_tools else None),
    )
    monkeypatch.setattr(
        subprocess,
        "run",
        lambda args, **_kwargs: calls.append(list(args)) or _docker_result(args),
    )
...
def test_node_install_uses_nodesource_on_linux(monkeypatch):
    calls = _run_node_install(monkeypatch, "Linux")

    assert any(
        "deb.nodesource.com/setup_22.x" in " ".join(call) for call in calls
    )
    assert any(
        call[:2] == ["apt-get", "install"] and "nodejs" in call for call in calls
    )


def test_node_install_uses_brew_on_macos(monkeypatch):
    calls = _run_node_install(monkeypatch, "Darwin", extra_tools=("brew",))

    assert ["brew", "install", "node"] in calls
    assert not any(call[0] in {"curl", "apt-get"} for call in calls)


def test_node_install_uses_winget_on_windows(monkeypatch):
    calls = _run_node_install(monkeypatch, "Windows", extra_tools=("winget",))

    assert ["winget", "install", "OpenJS.NodeJS.LTS"] in calls
    assert not any(call[0] in {"curl", "apt-get"} for call in calls)
...
def test_node_install_prints_platform_hint_without_package_manager(
    monkeypatch, capsys, system, hint
):
    calls = _run_node_install(monkeypatch, system)

    output = capsys.readouterr().out
    assert "[!]  Node.js not found" in output
